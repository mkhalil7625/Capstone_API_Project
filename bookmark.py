...
def bookmark_data(artist_name, image_url, song1, song2, song3, song4, song5, spotify_page_url, events):
    db = SqliteDatabase(database_path) # Get the path and connect to db
    db.connect()
    db.create_tables([Bookmarks])
...
#     artist_bookmark = Bookmarks.select()
#     return json.dumps(artist_bookmark)



def save_bookmarks(bookmark_id):
    responce, code = get_by_id(bookmark_id)
    if responce == 'Bookmark Not Found':
        add_artist()
